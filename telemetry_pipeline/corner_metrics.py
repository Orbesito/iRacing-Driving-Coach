...
def detect_main_corners(
    ...
    """
    Detect main corner apex points from the reference lap speed minima and define
    deterministic corner + phase boundaries.
    """
    ...
    # Multi-signal deterministic corner activity score:
    # speed deficit + yaw activity + lateral accel + steering demand.
    ...
    if "SteeringWheelAngle_deg" in ref.columns:
        steer_abs = (
            pd.to_numeric(ref["SteeringWheelAngle_deg"], errors="coerce").abs().fillna(0.0)
        )
    ...
    peak_candidates = peak_candidates.loc[
        peak_candidates["activity_score"] >= threshold
    ].copy()
    ...
    # Refine each activity peak to the nearest local speed minimum window for apex location.
    refined = []
    for _, peak in peak_candidates.iterrows():
        ...
        if window.empty:
            apex_dist = center
            apex_speed = float(
                ref.loc[(ref["distance_pct"] - center).abs().idxmin(), "speed_kmh_smooth"]
            )
        else:
    ...
    # Merge duplicate apex distances generated by neighboring peaks.
    ...
    if len(apex_distances) < config.min_corner_count:
        # Final deterministic fallback: strongest activity candidates.
        apex_distances = (
...
def compute_corner_metrics(
    ...
    """
    Compute per-corner per-lap metrics and aggregate coaching ranking outputs.
    """
            ...
            metric_rows.append(
                {
                    ...
                    "brake_peak_pct": _safe_max(brake_seg["Brake"])
    ...
    corner_lap_metrics = pd.DataFrame(metric_rows)

    ref_metrics = corner_lap_metrics.loc[
        corner_lap_metrics["lap_id"] == int(reference_lap_id)
    ].copy()
    ref_metrics = ref_metrics.rename(
        columns={
    ...
    corner_lap_metrics = corner_lap_metrics.merge(ref_metrics, on="corner_id", how="left")
    corner_lap_metrics["time_loss_vs_ref_s"] = (
        corner_lap_metrics["corner_time_s"] - corner_lap_metrics["ref_corner_time_s"]
    )
    corner_lap_metrics["apex_speed_loss_vs_ref_kmh"] = (
        corner_lap_metrics["ref_apex_speed_kmh"] - corner_lap_metrics["apex_speed_kmh"]
    )
    corner_lap_metrics["traction_delay_vs_ref_pct"] = (
        corner_lap_metrics["traction_reapply_delay_pct"]
        - corner_lap_metrics["ref_traction_reapply_delay_pct"]
    )

    comparison = corner_lap_metrics.loc[
        corner_lap_metrics["lap_id"] != int(reference_lap_id)
    ].copy()
    if comparison.empty:
        comparison = corner_lap_metrics.copy()
    ...
    corner_ranking = agg.reset_index()
    corner_ranking["std_time_loss_s"] = corner_ranking["std_time_loss_s"].fillna(0.0)
    corner_ranking["inconsistency_time_s"] = corner_ranking["inconsistency_time_s"].fillna(
        0.0
    )
    corner_ranking["mean_time_loss_s"] = corner_ranking["mean_time_loss_s"].fillna(0.0)
    corner_ranking["mean_apex_speed_loss_kmh"] = corner_ranking[
        "mean_apex_speed_loss_kmh"
    ].fillna(0.0)
    corner_ranking["mean_traction_delay_loss_pct"] = corner_ranking[
        "mean_traction_delay_loss_pct"
    ].fillna(0.0)

    norm_time_loss = _normalise_positive(corner_ranking["mean_time_loss_s"])
    norm_variability = _normalise_positive(corner_ranking["inconsistency_time_s"])
    norm_apex_speed_loss = _normalise_positive(corner_ranking["mean_apex_speed_loss_kmh"])

    corner_ranking["coaching_relevance_score"] = (
        0.60 * norm_time_loss + 0.30 * norm_variability + 0.10 * norm_apex_speed_loss
    )
    corner_ranking["coaching_priority_rank"] = (
        corner_ranking["coaching_relevance_score"]
        .rank(method="dense", ascending=False)
        .astype(int)
    )
    corner_ranking = corner_ranking.sort_values(
    ...
    report = {
        ...
        "laps_in_metrics": int(corner_lap_metrics["lap_id"].nunique()),
